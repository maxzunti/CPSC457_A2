...
void Scheduler::preempt() {               // IRQs disabled, lock count inflated
  ...
  if( affinityMask == 0 ) {
	  ...
	  target =  Runtime::getCurrThread()->getAffinity();
			...
			/* ADDED BY BRANKO */
		  int smallest = 10000; // arbitrary high numbers for finding the minimum
		  int smallestCore = 10000;
		  for (int i = 0; i < (Machine::getProcessorCount()); i = i + 1) {
		     unsigned int coreCheck = (0x1 << i);
		     if ((affinityMask & coreCheck) == coreCheck) { // AKA the core is useable
		        Scheduler *sched = Machine::getScheduler(i); // Get the scheduler for that core
		        if ((sched->readyCount) < smallest) { // If the ready count is the smallest encountered thus far
		           smallest = sched->readyCount; // Make it the core to be scheduled
		           smallestCore = i;
		        }
		     }
		  }

      //lowest ready count core found, now must assign the process to that core
      target = Machine::getScheduler(smallestCore); 
